    ...
    def do_GET(self):
        ...
        if self.path.startswith("/api/cabins/deckmap"):
            try:
                ...
                return self._json({"error": str(e)}, 500)
        # Mirrors routes/conga-line.ts GET /api/ships/:slug/rating — public,
        # only published+approved. 404 (not a placeholder) is the normal case
        # for most ships; the page renders nothing when this fails.
        if self.path.startswith("/api/ships/") and self.path.endswith("/rating"):
            try:
                slug = urllib.parse.unquote(self.path[len("/api/ships/"):-len("/rating")])
                if not slug:
                    return self._json({"ok": False, "error": "ship slug required"}, 400)
                rows = sb(
                    "conga_line_ratings?select=ship_slug,rating,rating_display,comment,grinch_take,"
                    "source_count,computed_at"
                    f"&ship_slug=eq.{urllib.parse.quote(slug)}"
                    "&status=eq.published&comment_status=eq.approved"
                )
                if not rows:
                    return self._json({"ok": False, "error": "No published rating for this ship"}, 404)
                row = rows[0]
                return self._json({
                    "ok": True,
        ...
        if self.path.startswith("/api/ships/ratings"):
            try:
                rows = sb(
                    "conga_line_ratings?select=ship_slug,rating,rating_display,comment,grinch_take,"
                    "source_count,computed_at&status=eq.published&comment_status=eq.approved&order=ship_slug"
                )
                return self._json({"ok": True, "ratings": rows})
            except Exception as e:
                return self._json({"ok": False, "error": str(e)}, 500)
        return super().do_GET()
