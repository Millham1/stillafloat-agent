...
//   salty_mark_take  — a SEPARATE, always-present dry/skeptical aside (the
...
2. "saltyMarkTake" — a SEPARATE one-liner, ONE sentence, in a DRYER, more skeptical register — the wry, well-traveled friend who's seen a hundred ships. This is NOT a rating and NOT part of the 1-5 scale; it sits ALONGSIDE the score as Mark's own independent aside. HARD RULES: (a) saltyMarkTake must NOT repeat, rephrase, or reference the same specific fact used in "comment" — pick a genuinely different real theme from what you were given. (b) saltyMarkTake must NEVER mention sources, review counts, ratings disagreeing, "the data," or anything that exposes how the score was built — it is Mark's own opinion, not a commentary on the composite. (c) saltyMarkTake must pull the OPPOSITE direction from "comment" — a true COUNTERPOINT, never agreement (Mark, 8/14: "counterpoint to the main observations, not agreement"). If the comment leads warm, Salty delivers the catch or the wrong-fit warning; if the comment leads with a warning, Salty names who'd still love it anyway. It must never soften, extend, restate, or pile onto the comment's point. Good angles: the catch nobody mentions, or WHO this ship suits vs. doesn't ("great if you like big and busy, skip it if you don't"). Humor always targets the ship's tradeoffs, never the traveler, never actually mean.
...
// about a weak ship lives in salty_mark_take, never in this number.
...
router.get("/ships/ratings", async (_req: Request, res: Response) => {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("conga_line_ratings")
      .select("ship_slug, rating, rating_display, comment, salty_mark_take, source_count, computed_at")
      .eq("status", "published")
      .eq("comment_status", "approved")
      .order("ship_slug");
    if (error) throw new Error(error.message);
...
// registers, returns a candidate rating + comment + saltyMarkTake. Does NOT save.
...
Write the "comment" and "saltyMarkTake" as specified.`;
    ...
    const out = JSON.parse(m[0]) as { comment?: string; saltyMarkTake?: string };
    if (!out.comment || !out.saltyMarkTake) throw new Error("incomplete draft (missing comment or saltyMarkTake)");
    ...
    return res.json({
      ...
      comment: out.comment,
      saltyMarkTake: out.saltyMarkTake,
    });
  } catch (err) {
...
router.post("/admin/conga-line/:slug/save", requireToken, async (req: Request, res: Response) => {
  try {
    ...
    if (!slug) return res.status(400).json({ ok: false, error: "ship slug required" });
    const body = (req.body ?? {}) as {
      rating?: number | string; ratingDisplay?: string; comment?: string; saltyMarkTake?: string; sourceCount?: number | string;
    };
    const rating = Number(body.rating);
    if (!Number.isFinite(rating) || rating < 1 || rating > 5) {
      return res.status(400).json({ ok: false, error: "rating must be between 1 and 5" });
    }
    if (!body.comment || !body.saltyMarkTake) {
      return res.status(400).json({ ok: false, error: "comment and saltyMarkTake are both required" });
    }
    ...
    const row = {
      ...
      salty_mark_take: body.saltyMarkTake,
...
router.post("/admin/conga-line/:slug/publish", requireToken, async (req: Request, res: Response) => {
  try {
    ...
    const supabase = getSupabase();
    const { data: existing } = await supabase
      .from("conga_line_ratings").select("ship_slug, comment, salty_mark_take").eq("ship_slug", slug).maybeSingle();
    if (!existing) return res.status(404).json({ ok: false, error: "No draft saved for this ship yet" });
    const row = existing as { comment: string | null; salty_mark_take: string | null };
    if (!row.comment || !row.salty_mark_take) {
      return res.status(400).json({ ok: false, error: "Draft is missing comment or saltyMarkTake — cannot publish" });
    }
